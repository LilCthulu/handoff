"""Security test fixtures — async DB, HTTP client, agent factories, crypto helpers.

Every security test gets a fresh SQLite database and a fully wired
httpx AsyncClient talking to the real FastAPI app via ASGI transport.
No mocks. No fakes. Real middleware. Real auth. Real database.
"""
...
os.environ["JWT_SECRET"] = "test-secret-key-for-security-audit"
...
async def client(db_engine):
    ...
    Overrides BOTH the get_db dependency AND the module-level async_session
    used by middleware (AuthMiddleware uses async_session directly).
    """
    session_factory = async_sessionmaker(db_engine, class_=AsyncSession, expire_on_commit=False)
    ...
    # Patch the module-level async_session used by AuthMiddleware.
    # Raise IP rate limits for tests — many agents register from 127.0.0.1 in rapid succession.
    import app.middleware.rate_limit as rl_module
    original_ip_max = rl_module.IP_MAX_REQUESTS
    rl_module.IP_MAX_REQUESTS = 500
...
class AgentFactory:
    ...
    async def create(
        ...
        """Register a new agent and return {agent, token, private_key, public_key, headers}.

        The returned dict has everything needed to act as this agent:
        - agent: full agent response dict
        - token: valid JWT bearer token
        - private_key: base64 Ed25519 private key (for signing)
        - public_key: base64 Ed25519 public key
        - headers: {"Authorization": "Bearer <token>"} ready to use
        """
...
def make_fake_token(
    agent_id: str = None,
    owner_id: str = "fake-owner",
    scopes: list[str] | None = None,
) -> str:
    """Create a valid JWT for a given agent_id (agent may not exist in DB)."""
